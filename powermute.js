...
(function(jQuery) {
    ...
    class _UI {
        ...
        hide_messages_with_name(name) {
            ...
            const messages = jQuery('#talks span.name')
                .filter((_, elem) => elem.textContent === name);

            talks.toArray().concat(messages.toArray()).forEach( elem => {
        ...
        create_messages_tab() {
            const tab_pm_messages = jQuery('<li role="presentation" id="settings-pm-messages-tab" class="">'
        ...
        create_pm_settings_tab() {
            const tab_pm_settings = jQuery('<li role="presentation" id="settings-pm-settings-tab" class="">'
                ...
                <div role="tabpanel" class="tab-pane" id="settings-pm-settings">
                    <div class="setting-content">
                        ...
                        <div id="pm-settings-enable" class="checkbox">
                            <label><div>
                                <input type="checkbox" id="checkbox-pm-settings-enable"` + (SETTINGS.is_enabled() ? 'checked' : '') + `>
                        ...
                        <div id="pm-settings-mute-no-trip" class="checkbox">
                            <label><div>
                                <input type="checkbox" id="checkbox-pm-settings-mute-no-trip"` + (SETTINGS.is_mute_user_if_no_tripcode() ? 'checked' : '') + `>
        ...
        create_list_rule_elem(userProp) {
            ...
            return jQuery(`<div class="input-group input-group-sm pm-rule-container" style="padding-bottom: 6px">
                <span for="pm-list-rule-` + text_id +  `" class="input-group-addon list-rule-span" style="min-width: 15%">`
                    + userProp.prop_type.description.toLowerCase() + `</span>
                <input type="text" id="pm-list-rule-` + text_id + `" name="list_rule" class="form-control form-inline input-sm" 
                    value="` + userProp.prop_val + `">
                <span class="input-group-btn">
                    <input type="button" name="play" class="btn btn-default btn-sm pm-list-rule-` + text_id + `" value="X">
                </span>
            </div>`);
        }
        ...
        populate_list_rules(list) {
            ...
            userProps.forEach( userProp => {
                jQuery('#settings-' + list.name + ' .setting-content')
                    .append( this.create_list_rule_elem(userProp) );
            });
        }
    }
